...
default_tickers = ["QQQ","SPY","NVDA","MSFT","MU", "AVGO","TSLA","AMD","PLTR","MRVL","uber","AMZN","AAPL","googl","META","MARA","nke","GM","c","wfc","hood","coin","bac","jpm","HIMS","TXM","QCOM","MU","INTC","CRDO","RMBS","ON","ORCL", "CRWD","PANW","APP","MSTR","IBM","AMAT","DELL","WDC","CRM","CHWY","ETSY","CART","W"]
...
# 3) Now that intraday is fully processed,
              ...
              # Remove rows with missing or invalid Time
                  ...
                  with st.expander("MIDAS Curves (Bull + Bear Anchors)", expanded=False):
                      ...
                      # ─────────────────────────────────────────────────────────
                      # 1)  Add Date + TimeIndex columns (safe if they already exist)
                      # ─────────────────────────────────────────────────────────
                      intraday["TimeIndex"] = pd.to_datetime(intraday["Time"], format="%I:%M %p", errors="coerce")
                      intraday = intraday[intraday["TimeIndex"].notna()].copy()
                      intraday["Date"] = intraday["TimeIndex"].dt.date
                      
                      # ─────────────────────────────────────────────────────────
                      # 2)  Tiny helper to turn 15-min slots into letters A, B, C…
                      # ─────────────────────────────────────────────────────────
                      letters = string.ascii_uppercase
                      def letter_code(n: int) -> str:
                      ...
                      # ─────────────────────────────────────────────────────────
                      # 3)  Loop day-by-day, build minimal profile, keep VA levels
                      # ─────────────────────────────────────────────────────────
                      daily_va = {}
                      
                      for d, df_day in intraday.groupby("Date"):
                          df = df_day.copy()
                      
                          # (a) F% binning (20-point bins)
                          f_bins = np.arange(-400, 401, 20)
                          df["F_Bin"] = pd.cut(df["F_numeric"], bins=f_bins,
                                               labels=[str(x) for x in f_bins[:-1]])
                      
                          # (b) Letter assignment
                          df["LetterIndex"] = ((df["TimeIndex"].dt.hour * 60 +
                                                df["TimeIndex"].dt.minute) // 15).astype(int)
                          df["LetterIndex"] -= df["LetterIndex"].min()
                          df["Letter"] = df["LetterIndex"].apply(letter_code)
                      
                          # (c) Count letters at each price bin
                          tpo_counts = (df.groupby("F_Bin")["Letter"]
                                        .apply(lambda s: ''.join(sorted(s.unique())))
                                        .reset_index())
                          tpo_counts["F% Level"] = tpo_counts["F_Bin"].astype(int)
                          tpo_counts["Letter_Count"] = tpo_counts["Letter"].str.len()
                      
                          # (d) Find POC (max letters)
                          poc = tpo_counts.loc[tpo_counts["Letter_Count"].idxmax(), "F% Level"]
                      
                          # (e) Build 70 % value area
                          target = 0.70 * tpo_counts["Letter_Count"].sum()
                          tpo_sorted = tpo_counts.sort_values("Letter_Count", ascending=False)
                          cum = 0
                          va_levels = []
                          for _, row in tpo_sorted.iterrows():
                              cum += row["Letter_Count"]
                      ...
                      if yesterday in daily_va:
                          ...
                          print("Yesterday's VAH / VAL / POC:", y_VAH, y_VAL, y_POC)
                      
                          # ➜  Add dashed lines (example for Plotly):
                          ...
                          print("No data for yesterday – cannot plot VA.")
...
# 1. Add the pink dotted line as a shape (visual line)
                ...
                for _, row in top_price_levels.iterrows():
                 ...
                 # # Create separate masks for upward and downward crosses:
                    ...
                    up_cross_trace = go.Scatter(
                        ...
                        y=intraday.loc[mask_kijun_up, "F_numeric"] + 16,  # Offset upward (adjust as needed)
                        mode="text",
                    ...
                    down_cross_trace = go.Scatter(
                        ...
                        y=intraday.loc[mask_kijun_down, "F_numeric"] - 16,  # Offset downward
                        mode="text",
 ...
 # Mask for each horse
  ...
  # correct masks
                ...
                fig.add_hline(y=y_VAH, line_dash="dash", line_color="green",
                            annotation_text="VAH (y-1)", annotation_position="top left")
              
                fig.add_hline(y=y_VAL, line_dash="dash", line_color="red",
                              annotation_text="VAL (y-1)", annotation_position="bottom left")
                
                fig.add_hline(y=y_POC, line_dash="dot", line_color="gray",
                              annotation_text="POC (y-1)", annotation_position="middle left")
