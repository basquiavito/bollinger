...
#                     # Add to figure
   ...
   #                      st.markdown("🟠 **Open Outside Range but NOT Accepted (possible fade or retest)**")
                ...
                bull_flip_trace = go.Scatter(
                    x=intraday["Time"],
                    y=intraday["F_numeric"].where(intraday["Compliance_Bull_Flip"] == "💨") + 36,
                    mode="markers",
                    marker=dict(size=30, symbol="triangle-down", color="#00ccff", line=dict(color="white", width=1)),
                    name="💨 Bull Compliance Flip",
                    hovertemplate="Time: %{x|%H:%M}<br>F%%: %{y:.2f}<extra></extra>"
                )
                ...
                bear_flip_trace = go.Scatter(
                    x=intraday["Time"],
                    y=intraday["F_numeric"].where(intraday["Compliance_Bear_Flip"] == "🌑") - 36,
                    mode="markers",
                    marker=dict(size=30, symbol="triangle-up", color="#ff4444", line=dict(color="white", width=1)),
