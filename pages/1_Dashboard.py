...
default_tickers = ["SPY","VIXY","SOXX","NVDA","MU", "AVGO","AMD","MRVL","SMCI","QQQ","uber","AMZN","MSFT","PLTR","AAPL","GOOGL","META","XLY","TSLA","nke","GM","c","DKNG","CHWY","ETSY","CART","W","KBE","wfc","hood","coin","bac","jpm"]
...
#                     # Add to figure
                ...
                entry1_mask = intraday["Entry1_Emoji"].str.contains("🎯1", na=False)
                
                fig.add_trace(go.Scatter(
                    x=intraday.loc[entry1_mask, "Time"],
                    y=intraday.loc[entry1_mask, "F_numeric"] + 244,
                    mode="text",
                    text=intraday.loc[entry1_mask, "Entry1_Emoji"],
                    textposition="top center",
                    textfont=dict(size=21),
                    name="🎯1 Entries (w/ Aids)",
                    hovertemplate="Time: %{x}<br>F%%: %{y}<extra></extra>"
                ), row=1, col=1)
