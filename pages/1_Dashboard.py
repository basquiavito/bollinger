...
default_tickers = ["SPY","QQQ","NVDA","SMCI","MU", "AVGO","AMD","PLTR","MRVL","uber","AMZN","AAPL","googl","META","MSFT","MARA","nke","GM","c","wfc","hood","coin","bac","jpm","HIMS","TXM","QCOM","MU","INTC","CRDO","RMBS","ON","ORCL", "CRWD","PANW","APP","MSTR","IBM","AMAT","DELL","WDC","CRM","CHWY","ETSY","CART","W"]
...
# 3) Now that intraday is fully processed,
                ...
                def detect_tenkan_pawns(df):
                    """
                    Enhanced version to detect Tenkan crosses more reliably.
                    Marks Pawn Down (♟️) or Pawn Up (♙) at Tenkan crosses.
                    Includes buffer zone handling and close price confirmation.
                    """
                    ...
                    # Calculate price closes for confirmation
                    close_prev = df["Close"].shift(1)
                    close_curr = df["Close"]
                    
                    # Detect crosses with small buffer zone (0.1% of price to handle floating point imprecision)
                    buffer = df["Close"].mean() * 0.001
                    
                    # Bullish cross: F was below Tenkan, now at or above (with close confirmation)
                    cross_up = (f_prev < t_prev - buffer) & (f_curr > t_curr + buffer) & (close_curr > t_curr)
                    
                    # Bearish cross: F was above Tenkan, now at or below (with close confirmation)
                    cross_down = (f_prev > t_prev + buffer) & (f_curr < t_curr - buffer) & (close_curr < t_curr)
...
# # ------------------------------------------------------------------------------------------------------------------------
                    ...
                    scatter_tk_sun = go.Scatter(
                        ...
                        y=intraday.loc[mask_tk_sun, "F_numeric"] + 180,  # Offset for visibility
                        mode="text",
                        text="🌞",
                        textposition="top center",
                        textfont=dict(size=34),
                    ...
                    scatter_tk_moon = go.Scatter(
                        ...
                        y=intraday.loc[mask_tk_moon, "F_numeric"] - 180,  # Offset for visibility
                        mode="text",
                        text="🌙",
                        textposition="bottom center",
                        textfont=dict(size=34),
                  ...
                  # (D) TD Trap Arrows - Only First Sell TD Trap
                    ...
                    scatter_bishop_tight = go.Scatter(
                        ...
                        y=intraday.loc[mask_bbw_tight, "F_numeric"] - 8,  # Adjusted Y offset
                        mode="text",
...
# 🟢  STD Expansion  (🐦‍🔥)
                    ...
                    scatter_std_alert = go.Scatter(
                        ...
                        y=intraday.loc[mask_std_alert, "F_numeric"] + 24,  # Offset above F%
                        mode="text",
...
#🟢 TD DEMAND
               ...
               # Extract only the rows where TDST just formed
                ...
                scatter_bishop_up = go.Scatter(
                    ...
                    y=intraday.loc[mask_bishop_up, "F_numeric"] + 32,
                    mode="text",
                ...
                scatter_bishop_down = go.Scatter(
                    ...
                    y=intraday.loc[mask_bishop_down, "F_numeric"] - 32,
                    mode="text",
                ...
                scatter_rook_down = go.Scatter(
                    ...
                    y=intraday.loc[mask_rook_down, "F_numeric"] - 24,  # Offset downward
                    mode="text",
...
# # #*******************************************************************************************************************************************************************************
                ...
                #     y=intraday.loc[mask_green_king, "F_numeric"] + 15,
                #     mode="text",
                ...
                #     y=intraday.loc[mask_red_king, "F_numeric"] - 15,
                #     mode="text",
