...
#**********************************************************************************************************************#**********************************************************************************************************************
   ...
   # Step 1: Calculate OBV
                ...
                def assign_dmi_emojis(df):
                    """
                    Adds three emoji signals based on DMI and Kijun cross logic:
                    🔦 scout_emoji — any +DI or -DI cross
                    🪽 wing_emoji — +DI cross within ±5 bars of Kijun up-cross
                    🦇 bat_emoji  — -DI cross within ±5 bars of Kijun down-cross
                    """
                    df = df.copy()
                    df["scout_emoji"] = ""
                    df["wing_emoji"] = ""
                    ...
                    # Identify Kijun crosses
                    kijun_up_cross = (df["F_numeric"] > df["Kijun"]) & (df["F_numeric"].shift(1) <= df["Kijun"].shift(1))
                    kijun_down_cross = (df["F_numeric"] < df["Kijun"]) & (df["F_numeric"].shift(1) >= df["Kijun"].shift(1))
                
                    # Get indices of Kijun crosses
                    ...
                    for i in range(1, len(df)):
                        ...
                        # Wing emoji: +DI cross AND Kijun up-cross within ±5 bars
                        if bullish_dmi_cross.iloc[i]:
                            nearby_kijun_up = any((i - 5) <= j <= (i + 5) for j in kijun_up_indices)
                            if nearby_kijun_up:
                                df.at[i, "wing_emoji"] = "🪽"
                
                        # Bat emoji: -DI cross AND Kijun down-cross within ±5 bars
